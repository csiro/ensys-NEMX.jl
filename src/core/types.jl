...
abstract type AbstractCPModel <: _PM.AbstractPowerModel end
mutable struct CPPowerModel <: AbstractCPModel
    _PM.@pm_fields
end
...
function get_fcas_participants_by_area(participants::Dict, service::FCASService, area::Int, area_gens)
    fcas_participants = filter(x -> fcas_enabled(service, x[2]), participants)
    ...
    participants = filter(x -> is_dispatchable(x[2]), fcas_participants)

    for (i, gen) in participants
        if !(gen["index"] in area_gens[area])
...
function target_by_area(target, target_area)
    fcas_target = Dict{String, Any}()
    for t in target
        if t["area"] == target_area
            fcas_target = t
        end
    end
    return fcas_target
...
function get_cost_data(model::_PM.DCPPowerModel, participant::Dict{String, Any})
    ...
    if (haskey(data, :mlf))
        ...
        price = Float64[]
        for i in 1:length(original_price)
...
"""
function get_cost_data(model::_PM.AbstractPowerModel, participant::Dict{String, Any})
    return participant["cost"]
end
