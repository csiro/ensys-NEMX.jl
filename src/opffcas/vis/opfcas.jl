...
Display a figure, but only where there is something to display it on.

PlotlyJS renders through an Electron window, which does not exist on a headless
machine — a CI runner, a batch job, a container. A plotting function that calls
`display` unconditionally therefore cannot be called from a script at all, which
is how a full 2000-bus OPFFCAS run used to die at its last line after twenty
minutes of solving.

So the display is attempted only in an interactive session, and any failure is
downgraded to a warning. The figure is always RETURNED either way, so a caller
can save it with `PlotlyJS.savefig` regardless of the environment.

Set `NEMX_NO_DISPLAY=1` to suppress the attempt even when interactive.
"""
function show_figure(fig)
    if isinteractive() && get(ENV, "NEMX_NO_DISPLAY", "0") != "1"
        try
            show_figure(fig)
        catch err
            @warn "could not display the figure; returning it instead" exception = err
        end
    end
    return fig
end
